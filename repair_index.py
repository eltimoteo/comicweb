...
<body class="h-screen overflow-hidden flex flex-col">
    ...
    <div class="flex flex-1 overflow-hidden">
        ...
        <div class="w-12 bg-[#333333] flex flex-col items-center py-4 gap-6 text-[#858585] text-xl border-r border-[#252526]">
            ...
            <i class="fa-solid fa-code-branch hover:text-white transition cursor-pointer activity-icon"></i>
            <i class="fa-solid fa-bug hover:text-white transition cursor-pointer activity-icon"></i>
            <div class="flex-1"></div>
            <i id="icon-settings" class="fa-solid fa-gear hover:text-white transition cursor-pointer mb-2 activity-icon"></i>
        </div>

        <!-- Sidebar (File Explorer) -->
        <div id="sidebar-panel" class="w-64 bg-[#252526] flex flex-col text-sm border-r border-[#1e1e1e] hidden md:flex">
    ...
    <!-- Settings Modal -->
    <div id="settings-modal" class="hidden absolute bottom-12 left-12 w-64 bg-[#252526] border border-[#454545] shadow-2xl z-50 text-sm">
        <div class="p-2 text-xs text-gray-500 font-bold uppercase tracking-wider border-b border-[#3e3e42]">Preferences</div>
        <div class="p-2">
            <div class="mb-2 text-[#d4d4d4]">Color Theme</div>
            <select id="themeSelect" class="w-full bg-[#3c3c3c] text-white p-1 border border-[#3e3e42] outline-none hover:border-[#007acc]">
        ...
        <div class="border-t border-[#3e3e42] p-2 hover:bg-[#2a2d2e] cursor-pointer text-[#d4d4d4] flex items-center justify-between group">
    ...
    <script>
        ...
        const themes = {
            'dark-plus': {
                ...
                '--border': '#3e3e42', '--accent': '#007acc', '--text-main': '#d4d4d4',
                '--keyword': '#569cd6', '--string': '#ce9178', '--function': '#dcdcaa'
            },
            'light-plus': {
                '--bg-editor': '#ffffff', '--bg-sidebar': '#f3f3f3', '--bg-activity': '#2c2c2c', 
                '--border': '#e4e4e4', '--accent': '#007acc', '--text-main': '#333333',
                '--keyword': '#0000ff', '--string': '#a31515', '--function': '#795e26'
            },
            'monokai': {
                '--bg-editor': '#272822', '--bg-sidebar': '#1e1f1c', '--bg-activity': '#171814', 
                '--border': '#1e1f1c', '--accent': '#f92672', '--text-main': '#f8f8f2',
                '--keyword': '#f92672', '--string': '#e6db74', '--function': '#a6e22e'
            },
            'abyss': {
                '--bg-editor': '#000c18', '--bg-sidebar': '#051336', '--bg-activity': '#102a5f', 
                '--border': '#2d3664', '--accent': '#77085a', '--text-main': '#6688c2',
                '--keyword': '#225588', '--string': '#22aa44', '--function': '#ddbb88'
            },
            'solarized': {
                '--bg-editor': '#fdf6e3', '--bg-sidebar': '#eee8d5', '--bg-activity': '#93a1a1',
                '--border': '#d3cbb7', '--accent': '#2aa198', '--text-main': '#586e75',
                '--keyword': '#859900', '--string': '#cb4b16', '--function': '#b58900'
            }
        };
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('icon-files').addEventListener('click', (e) => {
                sidebarExplorer.classList.toggle('hidden');
                document.querySelectorAll('.activity-icon').forEach(i => i.classList.remove('text-white', 'border-l-2', 'border-white'));
                if(!sidebarExplorer.classList.contains('hidden')) {
                    const icon = document.getElementById('icon-files');
                    icon.classList.add('text-white', 'border-l-2', 'border-white');
                }
            });
        ...
        function renderComics() {
            ...
            filtered.forEach(comic => {
                ...
                div.className = "vscode-card p-0 border-l-[3px] border-[#007acc] hover:border-white transition group relative overflow-hidden";
                div.style.backgroundColor = "var(--bg-sidebar)"; // Ensure var usage
                
                if(currentTab === 'marvel') div.style.borderColor = "var(--function)"; // Function Color (Yellow/Purple in default)
                if(currentTab === 'dc') div.style.borderColor = "var(--keyword)"; // Blue
                ...
                div.innerHTML = `
                    ...
                    <div class="p-3 font-mono text-xs">
                        <div style="color: var(--keyword)" class="mb-1">class <span style="color: var(--function)">${(comic.TITLE || '').replace(/\s+/g, '')}</span> {</div>
                        
                        <div class="pl-4 ml-1" style="border-left: 1px solid var(--border)">
                            <div><span style="color: var(--text-main)">author</span>: <span style="color: var(--string)">"${comic.WRITER}"</span>;</div>
                            <div><span style="color: var(--text-main)">year</span>: <span style="color: var(--number)">${(comic.PUBLISH || '').match(/\d{4}/)?.[0] || 'null'}</span>;</div>
                            <div><span style="color: var(--text-main)">read</span>: <span style="color: var(--keyword)">${isRead}</span>;</div>
                            ${comic.RATING ? `<div><span style="color: var(--text-main)">rating</span>: <span style="color: var(--number)">${comic.RATING}</span>;</div>` : ''}
                        </div>
